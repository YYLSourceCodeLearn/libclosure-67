...
// 根据对象的定义,凡是首地址是 *isa的结构体指针,都可以认为是对象(id), 在objc中,block实际上就是对象
// 编译器会根据block 捕获的变量,生成具体的结构定义. block内部的代码将会提取出来,成为一个单独的函数.创建block时,实际就是在方法中申明一个struct, 并且初始化该struct的成员. 而执行block时,就是调用那个单独的C函数,并把该struct指针传递过去.
    
// block中包含了被引用的自由变量(由struct持有), 也包含了控制成分的代码(由函数指针持有),符合闭包(closure)的概念
struct Block_layout {
    void *isa;
