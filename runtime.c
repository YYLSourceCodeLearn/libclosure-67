...
/*
 当struct第一次创建时,它是存在于该函数的栈帧上的, 其Class实固定的 _NSConcreteStackBlock. 其铺货的变量是会被赋值到结构体的成员上,所以当block初始化完成后,铺货到的变量不能更改
 当函数返回时,函数的栈帧被销毁, 这个block的内存也会被清除. 所以在函数结束后任然需要这个block时,就必须用Block_copy()方法将他拷贝到堆上. 这个方法的核心很简单:申请内存,将栈数据复制过去,将Class改一下, 最后捕获到的对象发送retain, 增加引用计数
